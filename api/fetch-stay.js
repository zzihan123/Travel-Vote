export default async function handler(req, res) {
  ...
  if (!ANTHROPIC_KEY) return res.status(500).json({ error: 'API 키가 설정되지 않았어요' });
...
반환 형식 (JSON만, 마크다운 코드블록 없이):
{
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: err?.error?.message || `Claude API 오류 ${response.status}` });
    }
